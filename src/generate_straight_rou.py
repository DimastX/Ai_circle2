...
def generate_straight_rou(N, eidm_params, q_fixed, start_speed=None, equilibrium_spacing=None):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write('    <vType id="car" accel="{:.2f}" decel="{:.2f}" sigma="{:.2f}" length="{:.2f}" minGap="{:.2f}" maxSpeed="{:.2f}" speedFactor="1.0" speedDev="0.0" guiShape="passenger" carFollowModel="IDM" tau="{:.2f}" delta="{:.2f}"/>\n'.format(
